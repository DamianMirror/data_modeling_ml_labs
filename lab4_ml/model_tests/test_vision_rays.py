...
def test_lidar_visualization():
    ...
    # 2. Проганяємо 50 кроків, просто тиснемо газ (дія 3)
    for i in range(45):
        action = 3  # Gas
    ...
    # Параметри променів (ті самі, що в агента)
    start_x, start_y = 48, 66  # Центр капота
    ray_length = 40
    ray_angles = [-0.5, 0, 0.5]  # Ліво, Центр, Право (радіани)
    ray_labels = ['Left (-0.5)', 'Center (0.0)', 'Right (0.5)']
    ...
    for angle in ray_angles:
        ...
        for i in range(ray_length):
            ...
            if px_value > threshold_val:
                ...
                # але в реальному агенті ми б зупинилися.
                # Для візуалізації зупинимось через пару пікселів після удару:
    ...
    ax1.set_title("Frame 50 + Lidar Rays")
    ...
    ax1.legend()
    ax1.set_xlim(0, 96)
    ax1.set_ylim(96, 0)  # Інверсія Y, бо в зображеннях 0 зверху
    ...
    for i, intensities in enumerate(rays_intensity_data):
        ...
        if hit_idx is not None:
            ...
            ax2.text(hit_idx, intensities[hit_idx] + 5, f"Hit at {hit_idx}", fontsize=8)
